...
- End of each session: 2–3 sentences in `DECISIONS.md` on what you
  ...
  "why Ridge over XGBoost for this," "why walk-forward not a single
  split," etc. Write it in your own words.
...
- Ridge regression per position (QB/RB/WR/TE separately — sample
  sizes are small, position usage patterns differ a lot).
- Rookies have no NFL history — decide now: build a separate
  ...
  and why.
- Compare against a naive baseline (e.g. last season's points) —
  if Ridge doesn't clearly beat naive, that's worth knowing, not
  hiding.
...
- Convert projections to draft rank via value-based drafting —
  points above replacement, replacement level set by your league's
  actual roster shape (10 teams, double FLEX matters here).
